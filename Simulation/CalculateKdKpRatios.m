...
%% HELPER FUNCTIONS: A_and_B_pendubot, pendubot_dynamics, hybrid_control_pendubot

DESIRED_ANGLE_DEGREES = 40; %Between 40 and 140 works best with LQR

DESIRED_ANGLE =DESIRED_ANGLE_DEGREES* pi/180;
...
x0 = [-pi/2; 0 ;0;0];
q1_ref_last = DESIRED_ANGLE;
tup_val =0.7;
q1_ref_initial = x0(1);
kp = 30;   
kd = 15;

%% Finding q1 reference trajectory --NO REFERENCE TRAJECTORY
% 
% syms t tup 
% q1_ref_traj = q1_ref_initial-(q1_ref_last-q1_ref_initial)*(-1/2+ 1/2*cos((t/tup) * pi));
% 
...
% 
% 
% q1_fun   = matlabFunction(q1_ref_traj,   'Vars', [t,tup]);
% dq1_fun  = matlabFunction(dq1_ref_traj,  'Vars', [t,tup]);
% ddq1_fun = matlabFunction(ddq1_ref_traj, 'Vars', [t,tup]);
% 
% t_vals = linspace(0, tup_val, 500);
...
q1_ref = q1_ref_last;
[A,B] = A_and_B_pendubot(q1_ref_last);
Q = [200^2 0 0 0; 
     0 50^2 0 0;
     0  0 5^2 0;
     0  0  0 5^2];
...
for i = 1:num_kd
        ...
        odefun = @(t,x) pendubot_dynamics(t, x, hybrid_control_pendubot(x,p,q1_ref,0,0,0,kp,kd_val,K,t,tup_val,1, false), p);
        ...
        for k = 1:length(q1)
            ...
            near_zero_q2 = (abs((q1_wrapped+q2_wrapped -pi/2)) < 0.3) ;
            slow_dq2     = abs(dq2(k)) < 0.05;
            q1_near_final = abs(q1_wrapped  -q1_ref_last) < .1;

            if near_zero_q2 && slow_dq2 && q1_near_final
                good_Tup = true;
                if abs((q1_wrapped+q2_wrapped -pi/2)) < best_Tup_q2 && abs(dq2(k))<0.05 &&t(k) >2
        ...
        if good_Tup
            ...
            subplot(3,1,2);
            plot(t(1:end), q2); hold on;
            yline(pi/2-q1_ref_last , '--',pi/2-q1_ref_last);
            yline(pi/2-q1_ref_last - 2*pi, '--',pi/2-q1_ref_last-2*pi);
            ylim([-2*pi 2*pi])
            ylabel('q2 (rad)');
            xlabel('time (s)');
...
odefun = @(t,x) pendubot_dynamics(t, x, hybrid_control_pendubot(x,p,q1_ref,0,0,0,kp,kd_Best,K,t,tup_val,1, true), p);
[t,X] = ode45(odefun, tspan, x0);
        q1  = X(:,1);
        q2  = X(:,2);
        dq1 = X(:,3);
...
figure;
subplot(2,1,1);
plot(t, mod(q1(1:end)+pi,2*pi)-pi ); hold on;
yline(q1_ref, '--', q1_ref);
ylabel('q1 (rad)');
title('Link 1 (actuated)');
